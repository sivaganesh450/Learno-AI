"""
LangGraph-based AI Agents for LearnO using Amazon Bedrock (Llama 3.3 70B)
...
# Initialize Amazon Bedrock LLM (used by most agents)
try:
    ...
    print(f"Successfully initialized Bedrock LLM (Llama 3.3 70B)")
except Exception as e:
    print(f"Warning: Could not initialize Bedrock LLM: {e}")
    llm = None

# Vision LLM — reuse the same Bedrock model
vision_llm = llm
...
class RoadmapAgent:
    """Agent for generating personalized learning roadmaps using Amazon Bedrock"""
    ...
    def _generate_roadmap(self, state: RoadmapState) -> dict:
        """Generate the learning roadmap using Bedrock"""
        topic = state["topic"]
...
Provide a week-by-week or phase-by-phase breakdown that fits within {duration}. Include specific topics, resources, projects, and milestones for each phase."""
        ...
        try:
            print(f"Calling Bedrock API for roadmap: {topic}")
            ...
            if response and hasattr(response, 'content') and response.content:
                ...
                if len(content) > 100 and "Phase" in content or "Week" in content or "#" in content:
                    print("Successfully generated roadmap from Bedrock")
                    return {
                        "messages": [AIMessage(content=content)],
                        "roadmap": content
                    }
                else:
                    print(f"Invalid response from Bedrock, using fallback. Response length: {len(content)}")
                    return self._fallback_roadmap(topic, skill_level, duration, skills_known)
            else:
                print("Empty response from Bedrock, using fallback")
                return self._fallback_roadmap(topic, skill_level, duration, skills_known)
...
class SummarizerAgent:
    ...
    def _answer_question(self, state: QAState) -> dict:
        """Answer user's question using Bedrock"""
        ...
        try:
            print(f"Calling Bedrock API for Q&A: {question[:50]}...")
            response = self.llm.invoke(messages)
            ...
            if response and hasattr(response, 'content') and response.content:
                ...
                # Check if response is valid
                if len(content) > 50:
                    print("Successfully generated answer from Bedrock")
                    return {
                        "messages": [AIMessage(content=content)],
                        "answer": content
                    }
                else:
                    print(f"Invalid response from Bedrock for Q&A, using fallback")
                    return self._fallback_answer(question)
            else:
                print("Empty response from Bedrock for Q&A, using fallback")
                return self._fallback_answer(question)
...
class QuizAgent:
    ...
    def load_history_from_chat(self, session_id: str, chat_messages: list):
        """
        Reconstruct quiz history from stored chat messages.
        This handles Lambda cold starts where in-memory session data is lost.
        Parses previous assistant messages to extract asked questions,
        domain/purpose/difficulty, and score.
        """
        import re
        session = self._get_session(session_id)
        
        # Only reconstruct if session history is empty but we have messages
        if session["history"] or not chat_messages:
            return
        ...
        for msg in chat_messages:
            ...
            # Extract question from quiz format: "📝 **Question N** (...)\n\n{question}\n\n**A)**"
            ...
            if question_match:
                ...
                questions_found.append(question_text)
                total_questions += 1
            
            # Track correct answers from evaluation messages
            if "✅ Correct!" in content:
                correct_count += 1
            
            # Extract domain/purpose/difficulty from session start message
            ...
            if domain_match and not session["domain"]:
                session["domain"] = domain_match.group(1).strip()
            if purpose_match and not session["purpose"]:
                purpose_text = purpose_match.group(1).strip().lower()
                if "interview" in purpose_text:
                    session["purpose"] = "interview"
                elif "exam" in purpose_text:
                    session["purpose"] = "exam"
                else:
                    session["purpose"] = "knowledge"
            if difficulty_match and not session["difficulty"]:
                session["difficulty"] = difficulty_match.group(1).strip().lower()
        ...
        session["questions_asked"] = total_questions
        session["score"] = correct_count
        print(f"[Quiz] Reconstructed {total_questions} questions from chat history for session {session_id}")
...
Type **"start"** when you're ready for your first question!"""
    ...
    async def generate_question(self, session_id: str) -> str:
        ...
        # Build context from ALL previous questions to avoid repetition
        prev_questions = [q["question"] for q in session["history"]] if session["history"] else []
        if prev_questions:
            prev_context = "\n".join([f"{i+1}. {q}" for i, q in enumerate(prev_questions)])
        else:
            prev_context = "None yet"
...
The following {len(prev_questions)} questions have ALREADY been asked in this session.
You MUST generate a COMPLETELY DIFFERENT question on a DIFFERENT topic/subtopic.
Do NOT rephrase or reword any previous question. Pick a new concept entirely.
    ...
    def _reason_through_problem(self, state: MathProblemState) -> dict:
        ...
        if not self.llm:
            return {
                ...
                "explanation": "Please ensure AWS Bedrock is configured."
            }
...
- Keep everything in PLAIN TEXT"""
    ...
    async def solve_from_image_stream(self, image_base64: str, session_id: str = "default"):
        ...
        if not vision_llm:
            yield "Error: Vision LLM not initialized. Please ensure AWS Bedrock is configured."
            return
...
**💡 Suggestions:**
- Make sure AWS Bedrock is configured correctly in your environment
...
class CodeAssistantAgent:
    ...
    Architecture (from diagram):
      User → Generate Code (Bedrock + CodeSolution schema)
           → Execute & Reflect
           → [attempt_count > MAX or error = None] → return to user
                                                   → [else] loop back to Generate Code

    Uses Amazon Bedrock (Claude 3 Haiku) — no hardcoded API keys.
    """
    ...
    def __init__(self):
        ...
        try:
            ...
            self.structured_llm = self.bedrock_llm.with_structured_output(CodeSolution)
            print("Code Assistant: Bedrock (Llama 3.3 70B) initialized")
        except Exception as e:
            print(f"Code Assistant: Bedrock init failed — {e}")
...
class DeepSearchAgent:
    ...
    Architecture (from diagram):
      INPUT (topic)
        ↓
      Report Planner (Bedrock + Web Search Tool)
        → Report Template with custom sections
        ↓
      [Parallel] Section Writer loop for each section:
          Researcher (Bedrock) ←→ Web Search Tool   →  Section Writer (Bedrock)
        ↓
      Format Sections
        ↓
      [Parallel] Final Section Writer (Bedrock) for each section
        ↓
      Compile Final Report  →  Final Report

    Uses Amazon Bedrock, TAVILY_API_KEY, DEEP_SEARCH_* from settings.
    """
    ...
    def __init__(self):
        ...
        # Bedrock LLM (general)
        self.bedrock = None
        # Bedrock LLM with structured output for planning
        ...
        try:
            ...
            self.planner_llm = self.bedrock.with_structured_output(ReportPlan)
            print("Deep Search Agent: Bedrock (Llama 3.3 70B) initialized")
        except Exception as e:
            print(f"Deep Search Agent: Bedrock init failed — {e}")
    ...
    # ------------------------------------------------------------------
    # Node 1: Report Planner (Bedrock structured output + Tavily context)
    # ------------------------------------------------------------------

    def _plan_report(self, state: DeepSearchState) -> dict:
        """
        Report Planner node.
        Uses Bedrock with structured output (ReportPlan) + a Tavily overview search.
        """
        topic = state["topic"]
    ...
    async def _research_section(self, section: ReportSection, topic: str) -> str:
        """
        Researcher node: Generate 2 search queries with Bedrock,
    ...
    async def _write_section(
        ...
        """
        Section Writer node: Uses Bedrock to write a polished section draft
        ...
        try:
            resp = await self.bedrock.ainvoke([
                SystemMessage(content=system_prompt),
                HumanMessage(content=user_prompt),
            ])
            return f"## {section.title}\n\n{resp.content.strip()}"
